...
export function PayrollDesk() {
  ...
  const load = async () => {
    try {
      const payrollResponse = await fetch("/api/payroll", { cache: "no-store" });
      const data = await payrollResponse.json().catch(() => ({}));
      if (!payrollResponse.ok) throw new Error(data.error ?? "Unable to load payroll data.");
      setProfiles(data.profiles ?? []);
      setStaff(data.staff ?? []);
    } catch (error) {
      setMessage({ kind: "error", text: error instanceof Error ? error.message : "Unable to load payroll data." });
    }
  };
  useEffect(() => { void load(); }, []);
