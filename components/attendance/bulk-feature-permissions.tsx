...
export function BulkFeaturePermissions({ accounts, onChange }: { accounts: StaffPermissionRow[]; onChange: (rows: StaffPermissionRow[]) => void }) {
  ...
  const departments = useMemo(() => Array.from(new Set(accounts.map((account) => account.department).filter(Boolean))).sort() as string[], [accounts]);
  const visible = department === "all" ? accounts : accounts.filter((account) => account.department === department);
  ...
  return <div className="rounded-2xl border border-border bg-background p-5">
    ...
    <div className="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">{visible.map((account) => <label key={account.staff_id} className={`flex cursor-pointer items-start gap-3 rounded-xl border p-4 transition ${selected.includes(account.staff_id) ? "border-primary bg-primary/5" : "border-border bg-muted/20"}`}><input type="checkbox" checked={selected.includes(account.staff_id)} onChange={() => toggleOne(account.staff_id)} className="mt-1 h-4 w-4 accent-primary" /><span className="min-w-0 flex-1"><span className="block truncate font-semibold">{account.staff_name}</span><span className="block text-xs text-muted-foreground">{account.department || "Unassigned department"}</span><span className="mt-3 flex flex-wrap gap-1.5 text-[11px]"><span className={`rounded-full px-2 py-1 ${account.leave_requests_enabled ? "bg-emerald-100 text-emerald-800" : "bg-muted text-muted-foreground"}`}>Leave {account.leave_requests_enabled ? "on" : "off"}</span><span className={`rounded-full px-2 py-1 ${account.planned_absence_enabled ? "bg-emerald-100 text-emerald-800" : "bg-muted text-muted-foreground"}`}>Planned {account.planned_absence_enabled ? "on" : "off"}</span><span className={`rounded-full px-2 py-1 ${account.attendance_exceptions_enabled ? "bg-emerald-100 text-emerald-800" : "bg-muted text-muted-foreground"}`}>Exceptions {account.attendance_exceptions_enabled ? "on" : "off"}</span></span></span>{selected.includes(account.staff_id) && <Check className="h-4 w-4 text-primary" />}</label>)}</div>
    ...
    {selected.length > 0 && <div className="mt-4 flex flex-wrap gap-2 border-t border-border pt-4"><button type="button" disabled={busy} onClick={() => void apply("leaveRequestsEnabled", true)} className="rounded-lg border px-3 py-2 text-sm font-semibold disabled:opacity-50">Enable leave</button><button type="button" disabled={busy} onClick={() => void apply("leaveRequestsEnabled", false)} className="rounded-lg border px-3 py-2 text-sm font-semibold text-muted-foreground disabled:opacity-50">Disable leave</button><button type="button" disabled={busy} onClick={() => void apply("plannedAbsenceEnabled", true)} className="rounded-lg border px-3 py-2 text-sm font-semibold disabled:opacity-50">Enable planned absence</button><button type="button" disabled={busy} onClick={() => void apply("plannedAbsenceEnabled", false)} className="rounded-lg border px-3 py-2 text-sm font-semibold text-muted-foreground disabled:opacity-50">Disable planned absence</button><button type="button" disabled={busy} onClick={() => void apply("attendanceExceptionsEnabled", true)} className="rounded-lg border px-3 py-2 text-sm font-semibold disabled:opacity-50">Enable exceptions</button><button type="button" disabled={busy} onClick={() => void apply("attendanceExceptionsEnabled", false)} className="rounded-lg border px-3 py-2 text-sm font-semibold text-muted-foreground disabled:opacity-50">Disable exceptions</button>{busy && <Loader2 className="h-5 w-5 animate-spin text-primary" />}</div>}
