...
interface FolioSummary { gross_amount: number | string; complimentary_amount: number | string; paid_amount: number | string; balance: number | string }
interface Authorization {
  ...
  folio_state?: "linked" | "pending_link";
}
      ...
      <Dialog open={Boolean(selected)} onOpenChange={(open) => !open && setSelected(null)}>
        <DialogContent className="max-h-[85dvh] overflow-y-auto sm:max-w-2xl">
          ...
          {selected && <div className="space-y-4 text-sm">
            ...
            {selected.folio_summary && <div className="rounded-xl border border-emerald-200 bg-emerald-50 p-4"><p className="font-semibold text-emerald-950">Default guest folio</p>{selected.folio_state === "pending_link" ? <p className="mt-2 text-sm text-emerald-800">Folio pending — link or activate a stay to use the app&apos;s default folio.</p> : <><p className="mt-1 text-xs text-emerald-800">Folio ID: {selected.folio_id}</p><div className="mt-3 grid gap-3 sm:grid-cols-4"><div><p className="text-xs text-emerald-800">Gross charges</p><p className="text-lg font-bold text-emerald-950">{formatCurrency(selected.folio_summary.gross_amount)}</p></div><div><p className="text-xs text-emerald-800">Complimentary used</p><p className="text-lg font-bold text-emerald-950">{formatCurrency(selected.folio_summary.complimentary_amount)}</p></div><div><p className="text-xs text-emerald-800">Paid</p><p className="text-lg font-bold text-emerald-950">{formatCurrency(selected.folio_summary.paid_amount)}</p></div><div><p className="text-xs text-emerald-800">Outstanding balance</p><p className="text-lg font-bold text-emerald-950">{formatCurrency(selected.folio_summary.balance)}</p></div></div>{selected.folio_items?.length ? <div className="mt-4 space-y-2 border-t border-emerald-200 pt-3">{selected.folio_items.map((item) => <div key={item.id} className="flex justify-between gap-3 text-sm"><span>{item.description} · {item.quantity} × {item.category}</span><span className="font-medium">{formatCurrency(item.total_amount)}</span></div>)}</div> : <p className="mt-3 text-sm text-emerald-800">No services, food, or beverage charges have been posted.</p>}</>}</div>}
