...
export async function printHotelReceipt(receipt: HotelReceiptData) {
  ...
  </style></head><body><main class="receipt"><header class="header">${receipt.hotel.logo ? `<img src="${escapeHtml(receipt.hotel.logo)}" alt="${escapeHtml(receipt.hotel.name)} logo">` : ""}<h2>${escapeHtml(receipt.hotel.name)}</h2>${receipt.hotel.address ? `<p>${escapeHtml(receipt.hotel.address)}</p>` : ""}${receipt.hotel.phone ? `<p>${escapeHtml(receipt.hotel.phone)}</p>` : ""}${receipt.hotel.email ? `<p>${escapeHtml(receipt.hotel.email)}</p>` : ""}<p>${escapeHtml(receipt.title)}</p></header><section class="meta"><p><strong>Receipt #:</strong><span>${escapeHtml(receipt.orderNumber)}</span></p><p><strong>Guest:</strong><span>${escapeHtml(receipt.guestName)}</span></p>${receipt.roomNumber ? `<p><strong>Room:</strong><span>${escapeHtml(receipt.roomNumber)}</span></p>` : ""}<p><strong>Date:</strong><span>${date.toLocaleString()}</span></p>${receipt.bookedBy?.name ? `<p><strong>Booked by:</strong><span>${escapeHtml(receipt.bookedBy.name)}${receipt.bookedBy.role ? ` (${escapeHtml(receipt.bookedBy.role)})` : ""}</span></p>` : ""}${receipt.checkedInBy?.name ? `<p><strong>Checked in by:</strong><span>${escapeHtml(receipt.checkedInBy.name)}</span></p>` : ""}${receipt.checkedOutBy?.name ? `<p><strong>Checked out by:</strong><span>${escapeHtml(receipt.checkedOutBy.name)}</span></p>` : ""}</section><hr>${items}<div class="total"><span>Total</span><span>GHS ${receipt.total.toFixed(2)}</span></div>${receipt.balance !== undefined ? `<div class="total"><span>Outstanding</span><span>GHS ${receipt.balance.toFixed(2)}</span></div>` : ""}<footer class="footer"><p>${escapeHtml(receipt.footer || "Thank you for choosing Kumbisaly Heritage")}</p></footer></main></body></html>`;
