        ...
        if (item.menu_item_id) {
          ...
          if (ingredients.length > 0) {
            ...
            for (const ingredient of ingredients) {
              ...
              console.log(`[InventoryManager] Deducted ${totalDeduction} ${ingredient.unit} of inventory item ${ingredient.inventory_item_id} for menu item ${item.menu_item_id}`);
            }
          }
        }
        ...
        if (!recipeFound) {
            ...
            if (invItemRes.rows.length > 0) {
                const invId = invItemRes.rows[0].id;
                 ...
                 console.log(`[InventoryManager] Direct deduction: ${item.quantity} for ${item.item_name}`);
            }
        }
      }
    });
