...
DO $$ BEGIN
    CREATE TYPE user_role AS ENUM ('admin', 'manager', 'staff', 'kitchen');
EXCEPTION
...
DO $$ BEGIN
    CREATE TYPE payment_method_enum AS ENUM ('cash', 'card', 'mobile');
EXCEPTION
    WHEN duplicate_object THEN null;
END $$;
...
CREATE TABLE IF NOT EXISTS order_items (
    ...
    notes TEXT,
    status order_status DEFAULT 'pending', -- Per-item status for kitchen
    created_at TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP
);
...
CREATE TABLE IF NOT EXISTS transactions (
    ...
    transaction_reference VARCHAR(255), -- External ID
...
CREATE TABLE IF NOT EXISTS kitchen_orderitems (
    ...
    preptime INTEGER,
    notes TEXT,
    menuitemid UUID REFERENCES menu_items(id) ON DELETE SET NULL
);
