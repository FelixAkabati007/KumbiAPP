  ...
  async function load() {
    ...
    if (review.ok) {
      ...
      if (staffResponse.ok) {
        ...
        setStaffAccounts((staffData.permissions ?? []).filter((item: { staff_id?: string }) => item.staff_id).map((item: { staff_id: string; staff_name?: string; leave_requests_enabled?: boolean; planned_absence_enabled?: boolean; attendance_exceptions_enabled?: boolean }) => ({ id: item.staff_id, name: item.staff_name || "Staff member", email: "", leave: item.leave_requests_enabled !== false, planned: item.planned_absence_enabled !== false, exceptions: item.attendance_exceptions_enabled !== false })));
      }
    }
