...
export default function AttendancePage() {
  ...
  const [staffAccounts, setStaffAccounts] = useState<{ id: string; name: string; email: string }[]>([]);
  ...
  async function load() {
    ...
    if (review.ok) {
      ...
      if (staffResponse.ok) {
        ...
        setStaffAccounts((staffData.permissions ?? []).filter((item: { staff_id?: string; staff_name?: string }) => item.staff_id).map((item: { staff_id: string; staff_name?: string }) => ({ id: item.staff_id, name: item.staff_name || "Staff member", email: "" })));
      }
    }
