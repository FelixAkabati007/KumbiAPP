...
export default function ComplaintsPage() {
  const { data, mutate } = useSWR<{ complaints: any[] }>("/api/complaints", fetcher);
  ...
  return <main className="min-h-screen bg-muted/20 p-4 sm:p-6"><div className="mx-auto flex max-w-7xl flex-col gap-6">
    ...
    <div className="grid gap-6 lg:grid-cols-[minmax(280px,360px)_1fr]">
      ...
      <section className="flex flex-col gap-4"><div className="flex items-center justify-between"><h2 className="text-xl font-semibold">My inbox</h2><Badge variant="secondary">{complaints.length} records</Badge></div>{complaints.length === 0 ? <Card><CardContent className="flex flex-col items-center gap-2 p-10 text-center"><CheckCircle2 className="text-primary" /><p className="font-medium">No complaints yet</p><p className="text-sm text-muted-foreground">Submitted concerns and manager responses will appear here.</p></CardContent></Card> : complaints.map((item) => <Card key={item.id}><CardHeader className="flex flex-row items-start justify-between gap-4"><div><CardTitle className="text-base">{item.title}</CardTitle><CardDescription>{item.department} · {new Date(item.created_at).toLocaleString()}</CardDescription></div><Badge variant={item.status === "resolved" || item.status === "closed" ? "secondary" : "outline"}>{statusLabels[item.status] || item.status}</Badge></CardHeader><CardContent className="flex flex-col gap-4"><p className="whitespace-pre-wrap text-sm leading-relaxed">{item.description}</p>{item.messages?.length ? <div className="flex flex-col gap-2 border-t pt-3">{item.messages.filter((m: any) => !m.is_internal).map((m: any) => <div key={m.id} className="rounded-lg bg-muted p-3 text-sm"><p>{m.message}</p><p className="mt-1 text-xs text-muted-foreground">{new Date(m.created_at).toLocaleString()}</p></div>)}</div> : null}<div className="flex flex-wrap gap-2">{["manager","admin"].includes(item.assignee_role) && <Button size="sm" variant="outline" onClick={() => update(item.id, "acknowledged")}>Acknowledge</Button>}{item.status !== "closed" && <Button size="sm" variant="outline" onClick={() => update(item.id, "resolved", "This complaint has been reviewed.")}>Mark resolved</Button>}</div></CardContent></Card>)}</section>
    </div>
  </div></main>;
}
