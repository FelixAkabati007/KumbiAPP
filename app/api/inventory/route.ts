...
export async function GET() {
  try {
    ...
    return NextResponse.json(res.rows.map((r: Record<string, unknown>) => ({ id: r.id, name: r.menu_item_name ?? r.name ?? "", sku: r.menu_item_barcode ?? r.sku ?? "", category: r.category ?? "ingredient", quantity: String(r.quantity), unit: r.unit ?? "units", reorderLevel: String(r.reorder_level ?? 0), cost: String(r.cost_price ?? 0), supplier: r.supplier ?? "", lastUpdated: r.last_updated ?? undefined, menuItemId: r.menu_item_id ?? undefined })));
  } catch (error) {
...
export async function POST(req: Request) {
  try {
    ...
    let res;
    if (menuItemId) {
      res = await query(`INSERT INTO inventory (menu_item_id, quantity, unit, reorder_level, cost_price, supplier, last_updated) VALUES ($1,$2,$3,$4,$5,$6,NOW()) ON CONFLICT (menu_item_id) DO UPDATE SET quantity=EXCLUDED.quantity, unit=EXCLUDED.unit, reorder_level=EXCLUDED.reorder_level, cost_price=EXCLUDED.cost_price, supplier=EXCLUDED.supplier, last_updated=NOW() RETURNING id`, [menuItemId, quantity ?? 0, unit ?? "units", reorderLevel ?? 0, cost ?? 0, supplier ?? null]);
    } else {
      if (!String(name ?? "").trim()) return NextResponse.json({ error: "Name is required for standalone inventory items" }, { status: 400 });
      res = await query(`INSERT INTO inventory (name, sku, category, quantity, unit, reorder_level, cost_price, supplier, last_updated) VALUES ($1,$2,$3,$4,$5,$6,$7,$8,NOW()) RETURNING id`, [String(name).trim(), sku ?? "", category ?? "ingredient", quantity ?? 0, unit ?? "units", reorderLevel ?? 0, cost ?? 0, supplier ?? null]);
    }
