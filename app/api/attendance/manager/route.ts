...
export async function GET() {
  ...
  try {
    const [pending, summary, frequency] = await Promise.all([
      query(`SELECT ar.id, ar.staff_id, COALESCE(NULLIF(TRIM(u.name), ''), NULLIF(CONCAT_WS(' ', sp.first_name, sp.last_name), ''), NULLIF(TRIM(u.email), ''), ar.staff_id::text) AS staff_name, sp.position, sp.job_classification, sp.department, u.email AS staff_email, COALESCE(u.role::text, 'staff') AS staff_role, ar.check_in_at, ar.check_out_at, ar.status, ar.verification_status, ar.created_at, CASE WHEN ss.scheduled_start IS NULL THEN 'Unscheduled' WHEN EXTRACT(HOUR FROM ss.scheduled_start AT TIME ZONE 'Africa/Accra') < 10 THEN 'Early' WHEN EXTRACT(HOUR FROM ss.scheduled_start AT TIME ZONE 'Africa/Accra') < 14 THEN 'Mid' ELSE 'Late' END AS shift_period FROM attendance_records ar LEFT JOIN staff_profiles sp ON sp.id = ar.staff_id LEFT JOIN users u ON u.id = sp.user_id LEFT JOIN staff_shifts ss ON ss.id = ar.shift_id WHERE ar.verification_status = 'pending' ORDER BY ar.check_in_at ASC LIMIT 100`),
