...
export async function GET(_request: Request, context: { params: Promise<{ receiptId: string }> }) {
  ...
  try {
    ...
    const levyResult = snapshot.subtotal != null ? calculateTaxes(Number(snapshot.subtotal), taxConfiguration, receipt.receipt_type === "event_booking" ? "events" : "rooms") : { breakdown: {} };
    const levyRows = levyRowsHtml(levyResult.breakdown, taxConfiguration, Number(snapshot.subtotal || 0), "GHS ");
