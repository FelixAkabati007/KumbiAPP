...
export async function POST(request: Request) {
  ...
  try {
    const result = await transaction(async (client) => {
      ...
      const receipt = await client.query(`INSERT INTO hotel_receipts (order_id, order_number, receipt_type, snapshot, created_by) VALUES ($1, $2, 'event_booking', $3::jsonb, $4) RETURNING id`, [event.id, `EVENT-${event.id.slice(0, 8).toUpperCase()}`, JSON.stringify({ event, quote, subtotal: Number(quote.subtotal) || 0, tax: Number(quote.tax_amount) || 0, total: Number(quote.total) || 0, levyLabel: 'GRA E-VAT / statutory levies', securedBy: { id: session.id, name: session.name, email: session.email, role: session.role }, securedAt: new Date().toISOString() }), session.id]);
